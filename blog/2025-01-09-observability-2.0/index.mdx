...
This is absolutely true. Metrics *alone* are insufficient to really understand the behavior of your system. You absolutely need high-cardinality data such as logs and traces to answer many types of questions. But I would argue that traditional metrics are still necessary as a *complement* to wide events.

Let's start with a challenge to the assumption that you can just collect *everything* as wide events, and then decide what questions you want to ask later. Regardless of which signal you're using, or how good your OTEL auto-instrumentation libraries are, you have important decisions to make about how to instrument your *domain-specific code*, and these decisions *always involve trade-offs*.

* With metrics, you get an incredibly simple, concentrated, ready-to-consume signal that costs *very, very little* (more on why this is important in a bit). The tradeoff is that you lose context of the individual events from which the aggregate measurement was derived.
* With traces, on the other hand, you get all the context for each individual event, but at at a cost of generating a *lot more data*: data you have to generate, encode, buffer, and transmit, and ultimately store. 

#### High-frequency events
When you're talking about instrumenting very high-volume/high-frequency events, the fundamental overhead of logs/traces can make them impractical in terms of computation, network, or storage.

For example: imagine an operation that happens 1000 times per second. You can instrument this with a single histogram metric, at the cost of a few active series. With traces, you're producing *1000 spans per second* for the same operation.

#### Instrumentation granularity

Even putting high-frequency events aside, you have choices to make up front about the granularity of your instrumentation. Given the overhead (and cost) of emitting spans, you have to be a lot more judicious about when you use them compared to metrics, which are so cheap that you can afford to use them much more liberally.

Having the ability to choose the most effective signal for a given use-case can give you a lot more flexibility to find a good balance between the richness of telemetry data and its cost.

#### The historical inertia of metrics

There's a couple other *eminently practical* reasons which make it hard to imagine giving up traditional metrics entirely: 

* Metrics are boring, simple, tried and true. This is valuable, if for no other reason than to have something to corroborate the metrics you derive dynamically from logs/traces.
...
I've hinted at cost being a big difference between metrics and traces/logs, but let's take another minute to consider just how important this is.

### Let's talk a bit more about the cost discrepancy

Metrics are *extraordinarily cheap*. Like a *fraction* of the cost of logs or traces, and the comparison gets more and more one-sided as you scale up.
...
With metrics, cost is driven by the number of timeseries you're producing: unique combinations of label/value pairs (e.g. `pod`, `request_path`, `response_code`). These pairs get stored once, and then the rest of the cost is basically just a number stored every interval (30 seconds or so).

If you handle 10x the number of requests without increasing the number unique combinations of labels you're measuring, you're still producing the same number of time series- the aggregate values will just be higher per data point. This would have no additional cost.
...
For example, if you only cared about the aggregate request count across all instances (and not *per instance*), you could aggregate away the `instance` label, and you'd suddenly have *one single series* to track all instances for a given operation.
:::

Traces and logs are fundamentally a more heavyweight signal, since each event is basically its own bag of string attributes. This is a lot more expensive than a metric series consisting of an initial bag of attributes, followed by only a number every interval. 

Vendors charge for each span/event (or by number of bytes) ingested, and so costs are based on the sum of all events you produce across all instances, over time. But unlike metrics, there's no aggregation, so there's nothing to bend the curve in your favor as your load increases.

This all adds up to make your CFO very unhappy with the idea of replacing metrics with another signal that's *an order of magnitude more expensive* and has a much more unpredictable growth curve.

Luckily, we have a way to control cost growth for traces: *sampling*.
