...
For example: imagine a pod which handles 1000 requests per second. You can instrument this with a single exponential histogram metric, at the cost of a few active series (lets say 10 for the sake of argument). With traces, you're producing *1000 spans per second*: one per request.

* With metrics, you have 10 active timeseries. Using [list pricing for Grafana Cloud](https://grafana.com/pricing/) of $8 for 1K series, this will cost **8 cents per month**.
...
* With metrics, you'll now have 100 active timeseries, which will cost **80 cents per month**.
