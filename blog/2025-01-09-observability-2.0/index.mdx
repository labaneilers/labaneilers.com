...
In the example above, if you only cared about the request duration percentiles across all pods (and not *per pod*), you could aggregate away the `pod` label, resulting in a single histogram- at a cost of **8 cents per month regardless of how many pods you scale to**.
:::
...
You may think the cost scaling example above is contrived, so let's talk about the real world. For SimpliSafe, the cost of capturing all our traces would be an **order of magnitude more expensive** than what we're currently paying for all our timeseries metrics.
...
So for these forensic use cases, we end up relying on traditional structured logs, and not traces. It's not looking good for Observability 2.0's reduction of duplicate pillars!
...
If we actually want to discard the three pillars model, we've *got to make wide events more cost efficient and affordable*. Until we can get the costs down, sampling will continue to be necessary, and sampling undermines the idea that we can discard the three-pillars model:
...
I think that sampling traces is actually a great tradeoff. You get most of the value, and you pay a fraction of the cost. 
