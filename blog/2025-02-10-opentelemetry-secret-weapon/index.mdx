...
:::tip[TL;DR]
As OpenTelemetry's adoption has surged, it's also drawn more criticism: it's complex, isn't fully matured, and its user experience can feel... unpolished. While these are valid criticisms, I think we've hit an inflection point where OTel's benefits outweigh its pain points, especially when compared to the alternative of proprietary telemetry pipelines and lock-in with the dominant (and outrageously expensive) vendors.

OTel's momentum has recently intensified due to the increased adoption of its secret weapon: its semantic conventions. These conventions allow all observability vendors to create the same rich, powerful, out-of-the-box user experience that the dominant vendors had locked-down via their ownership the entire telemetry pipeline.
:::
...
Now that OpenTelemetry (a.k.a. OTel) has gained such significant traction, it's starting to attract a lot of attention beyond the hardcore observability community. While most of what I read about OTel is pretty positive, it also draws its fair share of shade. 

Honestly, a lot of the criticism is pretty reasonable stuff. As a big user of OTel, I've spent plenty of hours rage debugging OTTL filters in OTel collectors, desperately searching for SDK examples that actually work, or pulling my hair out trying to figure out which version of a protobuf schema changed and broke telemetry from my Swift clients. And like a lot of the haters, I also get frustrated that the overall level of complexity in the specifications leaks into the implementation details of every part of the project.

All that said, I'm incredibly thankful that OTel exists. While it's still in its awkward teenage years, the fundamental shape of it has begun to solidify. Despite the challenges that currently exist, I think **OpenTelemetry is the only reasonable path forward for observability**.
...
* Any intermediate collection tools that collect the data, process/filter it, and forward it to a backend (e.g. DogStatsD, LogStash, Fluent Bit, Telegraf, etc)
:::

The reason OTel exists is that the tools available before it were mostly proprietary and vendor-specific. This has some big downsides:

* **Black boxes**: Telemetry agents/libraries/SDKs (the thing you install into your app to collect metrics, logs, etc) were closed source, not modifiable or verifiable by users. If they didn't they work *exactly* as you need, you could really only hope you could persuade the vendor to add a feature for you.
* **Vendor lock-in**: Since the agents are vendor-specific, the sheer effort required to switch vendors made it extremely cost prohibitive, which gave vendors enormous leverage to develop predatory pricing strategies  
...
The key to how this happened isn't particularly obvious. Clearly, there was initially an advantage to smaller vendors adopting OTel as a competitive differentiator. This put pressure on the industry, which slowly moved up-market until even the most dominant vendors felt pressure to offer at least some support (or lip-service for support).

## The actual threat of OpenTelemetry

There's a less obvious advantage that the dominant vendors had for years before OTel: *their ownership of the entire telemetry pipeline, end-to-end*. 
...
4. About a year later, realize now that you've fully integrated their proprietary telemetry pipeline, you're completely fucked when negotiating the contract renewal
:::

OTel directly threatens this advantage by giving us the ability to create our own telemetry pipelines. But there's a secret weapon, hidden in the list of stuff above that OTel does, which I think is the most powerful and underappreciated part of the project: 
...
This is why I think of OpenTelemetry's semantic conventions as a secret weapon. Now *all observability vendors* have the ability to go beyond just basic OTLP ingestion; they can now add this kind of rich, powerful, out-of-the-box user experience powered by an understanding of the meaning of the data they're collecting.
...
Like most technical choices, this is fundamentally about tradeoffs, and I think OpenTelemetry has hit the inflection point where its benefits have begun to outweigh its pain points for many, many situations. I'm acutely aware that there's still a lot of friction in getting OTel set up, you've got to compare it to the alternatives.

OpenTelemetry has finally become a workable enough solution that allows an organization to develop a solid observability capability - without having to trade-in their firstborn child in exchange for a few months of custom metrics.

And OTel is only going to get better. So many brilliant people are contributing, vendors are investing heavily, and adoption is surging. 
