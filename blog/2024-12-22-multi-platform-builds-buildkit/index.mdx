...
We ended up building an autoscaling, docker-compatible build service using BuildKit, which could build multi-platform container images, and then using Karpenter to auto-provision Graviton-based nodes.
...
Well, for one, you'ds see a roughly **30% improvement in price/performance** versus instances with x86 chips. This performance difference is even more pronounced at higher utilization, because unlike x86 chips, a Graviton vCPU is an *actual* CPU core, NOT a hyperthread you're sharing on a core with some rando. 
