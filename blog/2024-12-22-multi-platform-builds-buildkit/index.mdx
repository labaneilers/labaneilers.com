...
* `--cache-to` tells Docker to store the resulting cache of this build. The `mode=max` attribute is very important here, since otherwise the cache would only contain the final build stage layers. 
