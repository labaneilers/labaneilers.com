...
:::tip[TL;DR]
We wanted to take advantage of the cost savings and performance improvements of AWS's Graviton processors (ARM64), but wanted to do it incrementally. We ended up building an autoscaling, docker-compatible build service using Buildkit, which could build multi-platform container images, and then using Karpenter to auto-provision Graviton-based nodes.

This is paying off- we're seeing the expected 30% better performance per EC2 dollar spent, as well as some suprising benefits to developer productivity.
:::
...
AWS's Graviton is a 64 bit ARM-based CPU which is available only on EC2. Why, you ask, would one want to use Graviton-based instances when trusty old x86 instances have served us so well in the past?

Well, for one, you'd see a roughly **30% improvement in price/performance** versus instances with x86 chips. This performance difference is even more pronounced at higher utilization, especially when the memory bus gets saturated, because unlike x86 chips, a Graviton vCPU is an *actual* CPU core, NOT a hyperthread. 

{/* truncate */}


While you wont, as an AWS customer, directly see the correspondingly significant reduction in power consumption and carbon emissions, you can be assured it correlates directly with cost savings. You can save money whilst congratulating yourself for being a dedicated protector of the environment. Bonus!

At SimpliSafe, we decided that yes, we do enjoy saving money, and that we should probably look at how hard it would be to migrate our workloads to Graviton instances.

## How hard could recompiling everything be?

At SimpliSafe, we've got a heterogenous fleet of microservices built with a bunch of different languages. While we were eager to take advantage of the cost savings of Graviton instances, we weren't about to plunge headlong before we'd incrementally built some experience and confidence with it. We needed an approach that would support both ARM64 and AMD64 for some period, so that we could run a reasonable bake-off.
...
We believe strongly that developers need be able to build, test, and debug locally on their laptops, in an environment that's as close as possible to production. Whatever solution we came up with had to work for both Intel and ARM, both in AWS and on developer laptops.
...
### Docker Desktop with QEMU/Rosetta emulation

Docker Desktop (which we use for local development on our Macbooks) has features to enable CPU emulation for the internal Linux VM it uses to build and run containers (using either QEMU or Rosetta). It *usually* works pretty well, but it's far from perfect. There's a number of cases we ran into, especially with Rust and .NET builds, where the compiler would choke when run under emulation.

Even if emulation had been reliable, the performance hit when running under emulation varied from slightly annoying (e.g. node.js, Python) to mildly aggrivating (Go, .NET), to unbearably slow (Rust).

### Cross compilation

Many modern languages support cross-compilation (i.e. you can compile a binary for a different architecture than the one the compiler is running on). We toyed around with this option, but quickly realized what a mess it would be, given the breadth of languages/toolchains we use.
...
With either of these options, we *still* haven't gained the capability to build multi-platform images. With traditional Docker builds, you can only build single-architecture images. We really wanted to run per-service canary deployments to manage the risk of migration, and having to manage two separate per-architecture tagging schemes would have added a lot of additional complexity.

:::tip What are multi-platform container images?
Multi-platform container images are a pretty neat hack in the [OCI spec](https://github.com/opencontainers/distribution-spec/blob/v1.1.0/spec.md#listing-referrers) in which a specific image tag/digest points to a multi-platform "image index" instead of the manifest (list of layers) you'd get with a single-architecture image. This index is an additional layer of indirection: a list of digests pointing to manifests for different architectures. This allows a container runtime (e.g. Docker, containerd, cri-o, etc) to automatically choose the right image for the architecture it's running on.
:::
...
Our first iteration used Docker's [kubernetes driver](https://docs.docker.com/build/builders/drivers/kubernetes/), which given a kubeconfig context and a few other settings (e.g. resource requests/limits), will spin up BuildKit pods, and exec into them to run builds.
