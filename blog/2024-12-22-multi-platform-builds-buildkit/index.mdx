...
While you won't, as an AWS customer, directly see the correspondingly significant reduction in power consumption and carbon emissions, you can be assured it correlates directly with cost savings. You can save money whilst congratulating yourself for being a dedicated protector of the environment. Bonus!
...
Docker Desktop (which we use for local development on our Macbooks) has features to enable CPU emulation for the internal Linux VM it uses to build and run containers (using either QEMU or Rosetta). It *usually* works pretty well, but it's far from perfect. There's a number of cases we ran into, especially with Rust and .NET builds, where the compiler would choke hard (e.g. segfault!) when run under emulation.
