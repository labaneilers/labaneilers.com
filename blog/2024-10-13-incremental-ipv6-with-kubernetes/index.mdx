...
<a data-flickr-embed="true" href="https://www.flickr.com/photos/rsdreamphotos/3489532498/" title="So There&#x27;s Truth to This Whole &quot;Chipmunk Cheeks&quot; Thing..."><img src="https://live.staticflickr.com/3369/3489532498_47d124be37_w.jpg" width="400" alt="So There&#x27;s Truth to This Whole &quot;Chipmunk Cheeks&quot; Thing..."/></a>
...
It's fairly straightforward math: In AWS EKS, with the VPC CNI integration (i.e. a network plugin for Kubernetes that allows it to integrate with AWS's networking APIs), here's what it requires:

* The EKS control plane requires at least 16 addresses (at least 6 per subnet) 
* Every node (EC2 instance) requires at least one address, but depending on your CNI settings, they can eagerly allocate additional addresses to keep "warm" (to speed up pod creation)
...
This means, as we've migrated workloads to Kubernetes, we're increasing the number of IPs we're using by roughly **10x**.

This adds up quick. We've had a few close calls with IPv4 exhaustion during high traffic events where we had to scramble to temporarily kill non-critical workloads to free up IPs, rebalance across availability zones, or provision new subnets to make sure customers weren't affected.

## Actually, IPv6 is a thing

Unlike IPv4, IPv6 address space so incomprehensibly large that it's effectively unlimited. For example, a typical IPv6 *private subnet* would have a `/64` IPv6 CIDR, which is **18,446,744,073,709,551,616** addresses. 

:::tip fun fact
Apparently a number with that many digits is caled a "vigintillion". Numbers this large can only be discussed using your best Carl Sagan voice.
:::
...
It's legitimately really difficult to migrate a large distributed architecture like ours to IPv6, because, historically, it would require simultaneous changes across many different systems, along with some scary big-bang moments. It also requires reconsidering of a lot of assumptions built into your network design and security strategy. 
...
This translation of IP version between inside and outside the cluster has allowed us to migrate our workloads incrementally, which has made the whole process much more tractible.
...
Unfortunately, you can't enable IPv6 mode on an existing EKS cluster; you have to create a new cluster and migrate your workloads over. There have been a bunch of specific challenges around this (mostly just minutae around Terraform wrangling and executing DNS cutovers), but now that we've found all the sharp edges, the proces is pretty mechanical.
...
I've been a programmer for like 30 years, and I had never done anything with IPv6 before this migration. There were a few embarrasingly basic things I had to learn about IPv6: 
...
The same thing is true in .NET, Python, Rust, Java and probably most other languages that aren't doing something weird in their networking stack.
...
The nice thing about this is that the load balancer itself will listen on IPv4 addresses (in addition to IPv6 addresses).
