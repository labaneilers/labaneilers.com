...
One of the truisms we hear in software engineering leadership is that managers have a much bigger potential for impact on a business than an individual contributor. This is certainly a credible argument, given that a great manager can have a huge impact on a team. They're responsible for recruiting the right people, steering the culture, and making the biggest decisions about what risks to take, what opportunities to pursue, etc. Ultimately, they're accountable for what the team delivers.

And of course, a big team can deliver bigger outcomes, with bigger impact, than any individual contributor could on their own. Given this leverage, its no wonder that it's very rare for companies to have pay scales for individual contributors that match that of managers, especially into senior management. For those of us who find happiness and fulfillment in working directly with technology, our decision to avoid management can come with a significant economic penalty.

## The penalty for following your heart

I made the explicit decision a few years ago that I would leave management in order to get back to the things that originally drew me to this industry. While I still think of what I do as leadership, I've come to terms with the fact that for the remainder of my career, I'm going to watch my former peers surpass me in titles, power, and especially in compensation. 
...
But I want to push back a little on the idea that management is categorically more impactful than individual contribution. The concept is a bit of a tautology, because managers take credit for the innovation and impact of the individual contributors they hire. And because of the nature of software, a single person with the right idea, at the right time, can manifest that idea into the world without a ton of infrastructure or support.

I'd like to tell you the story of the most impactful thing I've ever done, which was as an individual contributor. Even though though this was 18 years ago, I honestly don't know if I'll have another chance to do something with quite such a big effect.
...
Let me just set the stage, and remind my readers what the web was like in 2006: Microsoft absolutely dominated the browser space since winning the so-called "browser wars" back in the late nineties. Chrome didn't exist. Firefox had been around for 4 years, but it held a fraction of the market share of IE (version 7 at the time), and had virtually no appreciable advantages in user experience over IE. JavaScript was widely regarded as a toy language, and both IE and Firefox had done virtually no innovation in their browsers' JavaScript engines, which were painfully slow. Honestly, none of us ever imagined JavaScript *could* be faster.
...
:::info[Comparison]
There are now lots of apps that allow users to do really sophisticated design in the browser (e.g. Canva, Figma). We didn't have any of the browser technologies back then that make this possible today: the `canvas` tag, WebAssembly, WebGL, or even half-decent JavaScript engines.
...
Around this time, a couple engineers had been working on Vistaprint's document rendering technologies, and had been working on a new set of server-side text rendering services that we could use for simpler products that didn't require Studio. I saw a demo that they'd built, and found myself unable to stop thinking about it for several days. One evening, while trying to get to sleep, I had a crazy idea.

What if we could build a brand-new Studio from scratch, where the document's elements would be composed of a set of server-rendered images? The client-side code would just be an interface for moving, resizing, end opening an editor for these elements, which from the perspective of the client, would just be rectangles with a set of properties. These elements would be the same types of elements users could edit in the legacy studio (e.g. text boxes, images, vector shapes, etc), but we'd build server-side rendering services for each one, which would output transparent PNG images.

The user could then just double click on any of the rectangles to open an element-specific editor. So for text boxes, this would open a simple text-box editor, which would allow the user to type, and then we'd debounce the keypress events to trigger a refresh of the server-rendered text. This way, the documents we produced client-side could be faithfully rendered server-side using the same text-layout engine.

## The pitch

The next day, I spent some time with the engineers who had done the text rendering work, and we started working through the details of the idea. Once we felt like we had something viable, we brought my boss, [Satish](https://www.linkedin.com/in/satishpai/), into the conversation. Having a shared experience of pain with Studio, he immediately arranged a meeting with [Wendy](https://www.linkedin.com/in/wendy-cebula-166a311/), Vistaprint's head of "Capabilities Development" (I don't remember the exact org chart at the time, but she was effectively the CTO).
...
:::info[Why not Flash?]
In the weeks that followed, I had to address a lot of questions from folks about why I had decided to use pure HTML/Javascript instead of Flash. It wasn't obvious at the time the Flash's days were numbered.

The real reason was, similar to with the IE text rendering engine, that we didn't want to build another server-side renderer that was dependent on Flash's proprietary text engine. It also wasn't clear that it was possible to use Flash server-side, or that Adobe wouldn't do something at any point to break our whole system.
:::
...
* The feel of the text editing, which we were most worried would be weird, was totally fine.
...
When the first A/B test came back, we were floored. Conversion rate was up by about 5 *points*. Out of the gate, this hack was immediately worth tens of millions of dollars a year for Vistaprint. And we had done *zero* optimization.
...
All this time, Vistaprint was growing like crazy. Each time we would move a product over to the new Studio, we'd see a huge jump in conversion rate. New core capabilities were being built on top the new Studio architecture, and enabled a ton of new design content, products, and features. Manufacturing could now be optimized much more efficiently given that we now had a reliable way to render documents that were faithful to the users' intentions.
...
Also: even though I had come up with the core idea for the new Studio, it was based on many of the ideas from the old Studio, which itself had required a lot of independent innovations. Beyond that, there's no way I could have come up with this idea, or had any hope of making it work, without the insight and skill of my teammates who had figured out the server-side rendering. 
...
Humility aside, I still occasionally catch myself thinking back to this achievement. It's hard to imagine how Vistaprint could had any semblance of the success they did if they had tried to continue with the original Studio architecture. 

I don't know how likely it will be that I'll play such a pivotal role in building a multi-billion dollar company in the future.

:::tip
In this case, I feel a great deal of gratitude to Wendy for having taken a risk in empowering me. It was a truly formative experience, and I feel lucky that the work I did had such a lasting effect on a great company.
:::

Bullets for what Wendy did to set the culture

## Innovation comes from individuals

Coming back to my original thesis, I think it's important to remember that innovation is almost always the product of *individuals*, not organizations. Managers who want to promote innovation have to work indirectly, through building a culture of empowerment and risk-taking, and fostering individuals with vision.
