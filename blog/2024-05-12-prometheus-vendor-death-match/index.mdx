...
* You don't have a lot of third party metrics sources (e.g. CloudWatch, ElasticSearch) that you want to query directly (Chronosphere integrates with those data sources through scraping them and converting them to Prometheus metrics)
...
That said, Grafana Cloud has been a great fit for us, and we're interested in expanding our use into their logging (Loki) and tracing (Tempo) products. I'll let you know how that goes.