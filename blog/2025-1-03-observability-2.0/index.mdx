---
slug: observability-2.0
title: Thoughts on Observability 2.0
...
)" was coined by Charity Majors (the CTO and co-founder of Honeycomb) as a shorthand for a new vision of observability, defined in opposition with the "three pillars" model. 

:::tip charity.wtf
If you're one of the 5 technology people on the internet that aren't already aware of [Charity's blog](https://charity.wtf/), you should really stop reading this drivel and head there immediately. She's a fountain of insight on technology, engineering management, observability, and many other topics.
...
* Collect telemetry in the form "wide events" (i.e. structured logs/traces which can contain an arbitrary number of key/value pairs with high cardinality values)
* Your backend should allow you to use these events to answer arbitrarily complex questions about your system
...
I may have oversimplified the argument against metrics; Charity's actual words are a bit nuanced. For instance, she still sees value in using metrics to monitor infrastructure (especially 3rd party infra); it's just that for the code that you're writing yourself- at the core of your business- and is changing all the time, metrics alone aren't sufficient to understand what's going on.

:::info of course I'd prefer wide events!
...
I may just be biased by the types of problems I've worked on, but I've found that there's a *lot* of aspects of the software I've written and maintained that are fairly constant over time. This is pretty self-evident if you consider the fact that we create alerts at all. For the most part, you know in advance what signals tell you when something's going off the rails.

Even after responding to an alert, I've historically found myself leaning on metrics when I first start investigating a problem. Because we've spent time thinking about the important signals in advance, metrics-driven dashboards are quickest way to get a read on the general shape of the problem. Only after that do I start formulating hypotheses and digging into logs and traces to look for confirmation.
...
Yes- we could be deriving these same metrics from logs or traces, but there's few remaining big factors which make me reluctant to abandon traditional metrics:

* Being forced to put thought into what an app's key metrics are- ahead of time- is actually a *pretty valuable exercise*. I find that when we rely on auto-instrumented spans, we tend to put less effort into thinking about how we're going to measure our core business concerns, and we end up not having spans in the right places when we need them.
* Metrics are boring and simple. Incrementing a counter or using histograms generated from some middleware is really easy to reason about, and much less prone to error (especially if you're utilizing sampling to try to control the cost of traces).
...
:::info Injest-time aggregation is a thing!
There's now a number of new, ingest-time aggregation tools (e.g. like Grafana Cloud's Adaptive Metrics or Chronosphere's aggregation rules), which can drastically reduce active series by aggregating away unneeded high-cardinality labels.
:::
...
* Logs are simple and ubiquitous. Everything writes logs. Also all OSS/3rd party infra produce logs, and very few produce traces.
* The developer experience for logs is great by default. Use `printf()`, start your app from a terminal, and watch the logs flow through stdout. It's a beautiful thing.

And then there's just the straight-up maturity and battle-tested quality of logs. As someone recently pointed out to me when I was talking through the prospect of going all in on traces: 
...
Honeycomb is indeed a fucking *pleasure* to use- it's super fast, very powerful, and intuitive to use. Honeycomb's existence has prompted a virtual tidal wave of innovation across the observability space as competitors struggle to react to its power and elegance.
...
To close the gap for a company like SimpliSafe, here's the gaps we'd need to close:

#### Affordability 

We've got to make wide events more cost efficient and affordable. Until we can get the costs down, sampling will continue to be necessary- which breaks diagnostic and forensic use cases. This makes the whole idea of discarding the other two pillars a non-starter. From my perspective, **sampling and Observability 2.0 are fundamentally incompatible**.

I'm seeing some pretty exciting things happening to make logs/traces more efficient, such as columnar databases like [Clickhouse](https://github.com/ClickHouse/ClickHouse), as well as other tracing projects like [Grafana Tempo](https://grafana.com/oss/tempo/) and [QuickWit](https://quickwit.io/). 

And I'm certainly not going to count out Honeycomb- they may have a few more tricks up their sleeve that could make a big difference in cost.

#### Developer experience

The developer tools available for tracing are legitimately a lot more complex than those available for logs. We'll need ubiquitous tracing developer tools that approach the ease of use of `printf()` debugging, and that make it just as easy to that validate your tracing instrumentation is working the way you intended.

The developer experience for OTEL SDK configuration for tracing is still... a work in progress. I hope to see more projects that package up OTEL SDKs and provide easier installation and configuration. Or who knows, maybe some of the SDK owners will decide that 200 lines of boilerplate before you can send a single span is a bit much.
...
As long as most infrastructure and 3rd party tools are emitting only logs and metrics, it's going to be hard to go all in on tracing. It's possible that OpenTelemetry's trajectory will continue, and more and more OSS tools will start emitting traces, but there's a lot of ground to cover to hit critical mass.

As a litmus test: an Observability 2.0 product would need some sort of drop-in Kubernetes monitoring solution similar to what you can get out-of-the-box with DataDog/NewRelic, or at least something comparable to the [Prometheus Operator](https://github.com/prometheus-operator/prometheus-operator).

## Being comfortable with incrementalism

Throughout my career, I've generally thought of incrementalism as a good thing. Perhaps Observability 2.0 isn't something we should aim to achieve in short order, but a longer journey, in which we can discover our way, step-by-step.
...
* Try to utilize each pillar for its strengths
* Trying to find sustainable ways to manage costs
