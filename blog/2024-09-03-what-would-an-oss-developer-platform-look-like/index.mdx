...
These values deeply inform the design of dex, and it's interesting to reflect on the iterations of platforms that I've built at different companies with different values, and how they were reflected in the design of the platforms.


## Example: ephemeral environments

Ephemeral developer environments are a feature I 100% know is a huge win for developers, regardless of your company culture. But for me, there have been big differences in design, features, and implementation details when I've built this feature at different companies.

### What are ephemeral environments?

Here's the gist: I want to be able to deploy an ephemeral environment from my local machine into Kubernetes, with a single command, so I can iterate, tweak, and test ideas in a production-like environment. I want a URL I can hit to I can play with usability, do manual testing, attach a debugger, troubleshoot config, etc. Then I want another command to tear the whole thing down.

I also want every feature branch I push to Github (or Gitlab, Bitbucket, etc) to trigger a CI job which deploys an ephemeral environment with the same properties, and have my automated tests run against


### Ephemeral environments in a financial institution

At my last job, in a commercial bank, we had a very small number of multi-tenant Kubernetes (OpenShift) clusters. As you might imagine in a highly-regulated industry, the creation of Kubernetes namespaces (and the associated access controls) was tightly controlled and managed by a central team.

Since we wanted to give developers the benefits of creating dynamic, isolated environments, we worked within the static structure of centrally managed namespaces by using strict naming conventions for any generated Kubernetes objects (e.g. prefixing all resources with the name of the developer or feature branch). 
