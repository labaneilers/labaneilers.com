...
<img src={StewImage} style={{width: 400, float: 'right', marginLeft: '20px'}} alt="The DevOps stew"/>
...
* A type of engineer that does the cloudy, opsy stuff (or a whole team of people that does the opsy stuff)
...
<img src={StewIngredientsImage} style={{width: 400, float: 'right', marginLeft: '20px'}} alt="Stew ingredients"/>
...
But as the braver teams started to actually practice continuous deployment, they found that there was an increase in the amount of regressions that would remain undiscovered, sometimes for days. In retrospect, our culture had been too reliant on having a QA team, who was organized around doing manual regression tests on big batches releaseEvents. Teams began to re-discover the need for some essential ingredients of continuous deployment:
...
It became increasingly obvious we needed to find a way to share the capabilities between teams, so we started experimenting with a new form of "platform" engineering- one that works in a world of autonomous teams and distributed systems.
...
Looking back, its actually hard for me to imagine, in any practical way, how any of these practices would exist on their own. I mean, you can gnaw on a potato, but its hard to call that a meal without the rest of the ingredients.
