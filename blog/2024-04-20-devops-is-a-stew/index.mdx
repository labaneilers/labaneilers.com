...
As the braver teams started to actually practice continuous deployment, they found that there was an increase in the amount of regressions that would remain undiscovered, sometimes for days. In retrospect, our culture had been too reliant on having a QA team, who was organized around doing manual regression tests on big batches releases. Teams began to re-discover the need for some essential ingredients of continuous deployment:
...
It became increasingly obvious we needed to find a way to share the capabilities between teams, so we started experimenting with ways to reclaim some of the abilities we used to have with our monolith- but in a way that worked in a world of autonomous teams and distributed systems.
